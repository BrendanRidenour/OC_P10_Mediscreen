...
namespace Mediscreen.Data
{
    ...
    {
        [Fact]
        public async Task GenerateDiabetesReport_PatientIdOverload_PatientServiceReturnsNull_ReturnsNull()
        {
        ...
        [Fact]
        public async Task GenerateDiabetesReport_PatientIdOverload_NoteServiceReturnsNull_ReturnsReport()
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
        ...
        [Fact]
        public async Task GenerateDiabetesReport_PatientIdOverload_WhenCalled_CallsTriggerTermsCounter()
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
            patientService.ReadById_Return = new()
        ...
        [InlineData(1)]
        [InlineData(2)]
        public async Task GenerateDiabetesReport_PatientIdOverload_WhenCalled_CallsDiabetesRiskAnalyzer(
            int triggerTerms)
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
        ...
        public async Task GenerateDiabetesReport_PatientIdOverload_WhenCalled_ReturnsReport(string givenName,
        ...
        [InlineData("famname1")]
        [InlineData("famname2")]
        public async Task GenerateDiabetesReport_PatientFamilyNameOverload_PatientServiceReturnsNull_ReturnsNull(
            string familyName)
        {
            var patientService = PatientService();
            patientService.ReadByFamilyName_Return = null;
            var service = Service(patientService);

            var result = await service.GenerateDiabetesReport(patientFamilyName: familyName);
        ...
        [InlineData("famname1")]
        [InlineData("famname2")]
        public async Task GenerateDiabetesReport_PatientFamilyNameOverload_NoteServiceReturnsNull_ReturnsReport(
            string familyName)
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
            patientService.ReadByFamilyName_Return = new()
            {
                Id = patientId,
                GivenName = "GN",
                FamilyName = familyName,
            ...
            var result = await service.GenerateDiabetesReport(familyName);

            Assert.Equal(patientId, noteService.ReadByPatientId_ParamPatientId);
            Assert.Equal($"Patient: GN {familyName} (age 20) diabetes assessment is: None", result);
        }

        [Fact]
        public async Task GenerateDiabetesReport_PatientFamilyNameOverload_WhenCalled_CallsTriggerTermsCounter()
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
            patientService.ReadByFamilyName_Return = new()
            ...
            await service.GenerateDiabetesReport(patientFamilyName: "FN");
        ...
        [InlineData(1)]
        [InlineData(2)]
        public async Task GenerateDiabetesReport_PatientFamilyNameOverload_WhenCalled_CallsDiabetesRiskAnalyzer(
            int triggerTerms)
        {
            var patientId = Guid.NewGuid();
            var patientService = PatientService();
            ...
            Assert.Equal(patientService.ReadByFamilyName_Return,
                diabetesRiskAnalyzer.AnalyzeRisk_ParamPatient);
            Assert.Equal(triggerTerms,
                diabetesRiskAnalyzer.AnalyzeRisk_ParamTriggerTermsCount);
        }
        ...
        [InlineData("GN3", "FN3", 1998, 22, 2)]
        [InlineData("GN4", "FN4", 1997, 23, 3)]
        public async Task GenerateDiabetesReport_PatientFamilyNameOverload_WhenCalled_ReturnsReport(string givenName,
            string familyName, int birthYear, int expectedAge, int expectedDiabetesRiskLevel)
        {
            var patientId = Guid.NewGuid();
            var patient = new PatientEntity()
            ...
            var patientService = PatientService();
            patientService.ReadByFamilyName_Return = patient;
            var noteService = NoteService();
            var triggerTermCounter = TriggerTermCounter();
            var diabetesRiskAnalyzer = DiabetesRiskAnalyzer();
            diabetesRiskAnalyzer.AnalyzeRisk_Return = (DiabetesRiskLevel)expectedDiabetesRiskLevel;
